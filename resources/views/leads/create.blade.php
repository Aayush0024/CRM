@extends('layouts.app')
@section('title', __('app.leads.add'))
@section('content')
<div class="py-2 max-w-3xl">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-xl font-bold text-gray-900">{{ __('app.leads.add') }}</h2>
    </div>
    <form action="{{ route('leads.store') }}" method="POST" class="space-y-5">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.leads.name') }} <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.leads.email') }}</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.leads.phone') }}</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.leads.company') }}</label>
                ...
                <div>
                    ...
                    <select name="status" class="w-full px-3 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        ...
                        <option value="{{ $s }}" {{ old('status','new') === $s ? 'selected' : '' }}>{{ __('app.status.'.$s) }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.leads.source') }}</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.leads.value') }}</label>
                    <input type="number" name="estimated_value" value="{{ old('estimated_value') }}" step="0.01" class="w-full px-3 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.leads.currency') }}</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.leads.close_date') }}</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.leads.customer_optional') }}</label>
                ...
                <div class="sm:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.leads.description') }}</label>
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('leads.index') }}" class="px-6 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium rounded-xl transition">{{ __('app.buttons.cancel') }}</a>
        </div>
    </form>
</div>
@endsection
