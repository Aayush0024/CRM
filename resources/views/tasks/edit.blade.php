...
<div class="py-2 max-w-2xl">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-xl font-bold text-gray-900">{{ __('app.tasks.edit') }}: {{ $task->title }}</h2>
    </div>
    <form action="{{ route('tasks.update', $task) }}" method="POST" class="space-y-5">
        @csrf @method('PUT')
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="sm:col-span-2"><label class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.tasks.title_field') }} <span class="text-red-500">*</span></label><input type="text" name="title" value="{{ old('title', $task->title) }}" required class="w-full px-3 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"></div>
                <div>
                    ...
                    <select name="type" class="w-full px-3 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        ...
                        <option value="{{ $t }}" {{ old('type', $task->type) === $t ? 'selected' : '' }}>{{ __('app.task_types.'.$t) }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="status" class="w-full px-3 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        ...
                        <option value="{{ $s }}" {{ old('status', $task->status) === $s ? 'selected' : '' }}>{{ __('app.status.'.$s) }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div class="sm:col-span-2"><label class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.tasks.description') }}</label><textarea name="description" rows="3" class="w-full px-3 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('description', $task->description) }}</textarea></div>
            </div>
        </div>
